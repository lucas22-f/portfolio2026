...
Define the FastAPI-owned contact submission boundary that validates the four-field form, applies bounded abuse and duplicate protection, delivers accepted submissions through Resend, and never persists or exposes submitted personal data outside the delivery operation.
...
- THEN it MUST attempt delivery through the configured Resend integration
- AND it MUST return a stable success outcome only after Resend accepts the message for delivery
...
- THEN it MUST reject the request without calling Resend
...
### Requirement: Non-persistent Resend delivery

The backend MUST deliver accepted contact submissions directly to the configured Resend recipient using backend-only configuration. It MUST NOT persist submissions, create an inbox or database record, or retain message content beyond the minimum transient processing required to complete the request.

#### Scenario: Accepted submission is delivered without application persistence

- GIVEN a valid, non-duplicate request and an available Resend configuration
- WHEN Resend accepts the outbound message
...
#### Scenario: Resend credentials remain backend-only

- GIVEN the contact-delivery service is configured
- WHEN the frontend receives form configuration or a submission response
- THEN no Resend API key, sender secret, recipient secret, or provider credential MUST be present in the response, chat event, or browser-visible configuration

#### Scenario: Delivery failure does not create a partial local record

- GIVEN Resend rejects, times out, or is unavailable while processing a request
...
- GIVEN validation, abuse, duplicate, configuration, or Resend failures occur
...
- THEN the backend MUST reject or defer the request without calling Resend
...
- THEN the backend MUST reject it before invoking Resend
...
- GIVEN a logical submission identity has already been accepted by Resend
- WHEN the same identity is submitted again within the duplicate-protection window
- THEN the backend MUST NOT call Resend a second time
...
#### Scenario: Resend acceptance returns success

- GIVEN validation and abuse checks pass
- AND Resend accepts the outbound message
...
- GIVEN Resend times out, is temporarily unavailable, or returns a transient failure
- WHEN the backend completes processing
...
- GIVEN Resend rejects the request for a permanent or non-retryable reason
- WHEN the backend completes processing
...
- GIVEN the Resend API key, recipient, sender, or required delivery configuration is absent or invalid
...
- THEN it MUST reject the request before calling Resend
