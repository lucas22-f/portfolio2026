...
describe('ChatClient SSE streaming', () => {
  it('processes an SSE frame fragmented across chunks in sequence order', async () => {
    ...
    globalThis.fetch = async () => response(new ReadableStream({
      start(controller) {
  ...
  it('rejects a second start frame after emitting the first', async () => {
    ...
    await expect(new ChatClient().stream('Consulta', (event) => events.push(event))).rejects.toMatchObject({
      code: 'invalid-provider-output', retryable: false,
    });
  ...
  it('rejects a terminally incomplete SSE stream as retryable', async () => {
    ...
    await expect(new ChatClient().stream('Consulta', () => undefined)).rejects.toMatchObject({
      code: 'stream-closed', retryable: true,
    });
    globalThis.fetch = originalFetch;
  });
  ...
  it('rejects incompatible content versions before rendering them', async () => {
    const originalFetch = globalThis.fetch;
    globalThis.fetch = async () => response(sse([{ ...start, content_version: 'different-version' }]));

    await expect(new ChatClient().stream('Consulta', () => undefined)).rejects.toMatchObject({
      code: 'content-incompatible', retryable: false,
    });
    globalThis.fetch = originalFetch;
  });
  ...
  it('requires the SSE response media type', async () => {
    ...
    await expect(new ChatClient().stream('Consulta', () => undefined)).rejects.toThrow('provider-unavailable');
    globalThis.fetch = originalFetch;
  });
});
