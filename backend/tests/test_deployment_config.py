...
from app.main import (
    ...
    _configured_origins,
    _configured_preview_origin_regex,
)
...
def test_vercel_builds_the_frontend_from_repository_root() -> None:
    ...
    assert (
        config["buildCommand"]
        == "cd frontend && npx ng build --define __API_BASE_URL__=\"'$API_BASE_URL'\""
    )
...
def test_deployment_environment_templates_keep_api_and_cors_values_non_secret() -> None:
    ...
    assert (
        "CORS_ALLOWED_ORIGINS=http://localhost:4200,https://portfolio2026.vercel.app"
        in backend_environment
    )
    assert (
        "CORS_PREVIEW_ORIGIN_REGEX=^https://portfolio2026(?:-[a-z0-9-]+)?\\.vercel\\.app$"
        in backend_environment
    )
